...
public final class InfoActivity extends Activity {
	...
	public void onCreate(final Bundle savedInstanceState) {
		...
		final String compatiblePackage = PackageUtilities.getCompatiblePackage(
				manager, null);
		...
		if (null != compatiblePackage) {
			...
			Log.v(Constants.LOG_TAG,
					String.format(
							Locale.US,
							"Locale-compatible package %s is installed", compatiblePackage)); //$NON-NLS-1$
			try {
				final Intent i = manager
						.getLaunchIntentForPackage(compatiblePackage);
			...
			Log.i(Constants.LOG_TAG,
					"Locale-compatible package is not installed"); //$NON-NLS-1$

			try {
				startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(String
						.format(Locale.US, APP_STORE_URI,
