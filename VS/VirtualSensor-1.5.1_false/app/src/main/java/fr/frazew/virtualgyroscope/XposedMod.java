...
public class XposedMod implements IXposedHookLoadPackage {
    ...
    public void handleLoadPackage(final LoadPackageParam lpparam) throws Throwable {
       ...
       Context context = (Context) AndroidAppHelper.currentApplication();
        if (context!=null){
            Log.d("UDP","CONTEXT!!!access!!!Xposed");
        }
    ...
    private void hookSensorValues(final LoadPackageParam lpparam) {
        if (Build.VERSION.SDK_INT >= 24)
            ...
            XposedHelpers.findAndHookMethod("android.hardware.SystemSensorManager$SensorEventQueue",
                    lpparam.classLoader, "dispatchSensorEvent", int.class, float[].class, int.class, long.class,
                    new fr.frazew.virtualgyroscope.hooks.sensorchange.AllApi());
            //XposedBridge.log("VirtualSensor: Using SDK version " + Build.VERSION.SDK_INT + ", this is not supported");
    ...
    private void addSensors(final LoadPackageParam lpparam) {
        ...
        else             XposedHelpers.findAndHookConstructor("android.hardware.SystemSensorManager",
                    lpparam.classLoader, android.os.Looper.class,
                    new fr.frazew.virtualgyroscope.hooks.constructor.AllApi(lpparam));
    }
