...
public class XposedMod implements IXposedHookLoadPackage {
    ...
    void Broadcastrecieve() {
        Context context = (Context) AndroidAppHelper.currentApplication();
        BroadcastReceiver br = new BroadcastReceiver() {
            // действия при получении сообщений
            public void onReceive(Context context, Intent intent) {
                float[] values = intent.getFloatArrayExtra(MainActivity.PARAMVALUENAME);
                Log.d("UDP", "values:" + Arrays.toString(values));
            ...
            protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                ...
                if (appContext != null) {
                    ...
                    if ((receiveSocket == null) && (isNetworkAvailable(appContext))) {
                        ...
                        receiveSocket = new ReceiveSocket(this);
    ...
    public static final SparseArray<SensorModel> sensorsToEmulate = new SparseArray<SensorModel>() {{
        ...
        if (Build.VERSION.SDK_INT >= 19)
            append(Sensor.TYPE_GEOMAGNETIC_ROTATION_VECTOR, new SensorModel(Sensor.TYPE_GEOMAGNETIC_ROTATION_VECTOR, "VirtualSensor GeomagneticRotationVector", sensorTypetoHandle.get(Sensor.TYPE_GEOMAGNETIC_ROTATION_VECTOR), 0.01F, -1, -1, (Build.VERSION.SDK_INT > 19) ? Sensor.STRING_TYPE_GEOMAGNETIC_ROTATION_VECTOR : "", "none"));
        ...
        if (Build.VERSION.SDK_INT >= 18)
            append(Sensor.TYPE_GAME_ROTATION_VECTOR, new SensorModel(Sensor.TYPE_GAME_ROTATION_VECTOR, "VirtualSensor GameRotationVector", sensorTypetoHandle.get(Sensor.TYPE_GAME_ROTATION_VECTOR), 0.01F, -1, -1, (Build.VERSION.SDK_INT > 19) ? Sensor.STRING_TYPE_GAME_ROTATION_VECTOR : "", "none"));
    }};
    ...
    private void hookSensorValues(final LoadPackageParam lpparam) {
        if (Build.VERSION.SDK_INT >= 24)
            ...
            log("VirtualSensor: Using SDK version " + Build.VERSION.SDK_INT + ", this is not supported");
    }
    ...
    private void enableSensors(final LoadPackageParam lpparam) throws XposedHelpers.ClassNotFoundError {
        if (Build.VERSION.SDK_INT >= 18) {
            try {
                ...
                log("VirtualSensor: Could not hook the AOSP enableSensor method, trying an alternative hook.");
                try {
                    ...
                    log("VirtualSensor: The alternative enableSensor hook failed, but the module might still work.");
                }
            }
        } else if (Build.VERSION.SDK_INT >= 16) {
    ...
    private void hookCardboard(final LoadPackageParam lpparam) {
        try {
            ...
            if (headTransformTMP == null) {
                ...
                if (headTransformTMP != null)
                    log("VirtualSensor: Did not find com.google.vrtoolkit.cardboard.HeadTransform but found com.google.vr.sdk.base.HeadTransform");
            }
            ...
            if (headTransform != null) {
                log("VirtualSensor: Found the Google Cardboard library in " + lpparam.packageName + ", hooking HeadTransform");
