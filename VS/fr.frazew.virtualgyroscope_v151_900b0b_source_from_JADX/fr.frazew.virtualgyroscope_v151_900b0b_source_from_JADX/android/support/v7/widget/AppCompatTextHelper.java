...
class AppCompatTextHelper {
    private static final int[] TEXT_APPEARANCE_ATTRS = new int[]{C0120R.attr.textAllCaps};
    ...
    void loadFromAttributes(AttributeSet attrs, int defStyleAttr) {
        ...
        a.recycle();
        if (ap != -1) {
            TypedArray appearance = context.obtainStyledAttributes(ap, C0120R.styleable.TextAppearance);
            if (appearance.hasValue(C0120R.styleable.TextAppearance_textAllCaps)) {
                setAllCaps(appearance.getBoolean(C0120R.styleable.TextAppearance_textAllCaps, false));
            }
            appearance.recycle();
        }
